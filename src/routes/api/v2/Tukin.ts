...
import z from "zod";
import { DataTukinControllerV2 } from "@/controllers/v2/tukin.controller";
...
router.get("/", validateQuery(querySchema), DataTukinControllerV2.getAll);
router.get("/Count", validateQuery(querySchema), DataTukinControllerV2.count);
router.get("/GetTahun", DataTukinControllerV2.getTahun);
router.get(
  "/Tahun/:tahun/GetBulan",
  validateParams(z.object({ tahun: z.string().regex(/^\d{4}$/, "invalid format tahun [YYYY]") })),
  DataTukinControllerV2.getBulan
);
router.get("/GetRekap", validateQuery(querySchema), DataTukinControllerV2.getRekap);
router.get("/:id", DataTukinControllerV2.getById);
