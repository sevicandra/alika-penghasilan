...
import z from "zod";
import { DataGajiControllerV2 } from "@/controllers/v2/gaji.controller";
...
router.get("/", validateQuery(querySchema), DataGajiControllerV2.getAll);
router.get("/Count", validateQuery(querySchema), DataGajiControllerV2.count);
router.get("/GetTahun", DataGajiControllerV2.getTahun);
router.get(
  "/Tahun/:tahun/GetBulan",
  validateParams(z.object({ tahun: z.string().regex(/^\d{4}$/, "invalid format tahun [YYYY]") })),
  DataGajiControllerV2.getBulan
);
router.get("/GetRekap", validateQuery(querySchema), DataGajiControllerV2.getRekap);
router.get("/:id", DataGajiControllerV2.getById);
