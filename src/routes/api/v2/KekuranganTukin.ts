...
import z from "zod";
import { DataKekuranganTukinControllerV2 } from "@/controllers/v2/kekuranganTukin.controller";
...
router.get("/", validateQuery(querySchema), DataKekuranganTukinControllerV2.getAll);
router.get("/Count", validateQuery(querySchema), DataKekuranganTukinControllerV2.count);
router.get("/GetTahun", DataKekuranganTukinControllerV2.getTahun);
router.get(
  "/Tahun/:tahun/GetBulan",
  validateParams(z.object({ tahun: z.string().regex(/^\d{4}$/, "invalid format tahun [YYYY]") })),
  DataKekuranganTukinControllerV2.getBulan
);
router.get("/GetRekap", validateQuery(querySchema), DataKekuranganTukinControllerV2.getRekap);
router.get("/:id", DataKekuranganTukinControllerV2.getById);
