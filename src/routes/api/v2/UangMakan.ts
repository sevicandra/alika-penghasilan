...
import z from "zod";
import { UangMakanControllerV2 } from "@/controllers/v2/uangMakan.controller";
...
router.get("/", validateQuery(querySchema), UangMakanControllerV2.getAll);
router.get("/Count", validateQuery(querySchema), UangMakanControllerV2.count);
router.get("/GetTahun", UangMakanControllerV2.getTahun);
router.get(
  "/Tahun/:tahun/GetBulan",
  validateParams(z.object({ tahun: z.string().regex(/^\d{4}$/, "invalid format tahun [YYYY]") })),
  UangMakanControllerV2.getBulan
);
router.get("/GetRekap", validateQuery(querySchema), UangMakanControllerV2.getRekap);
router.get("/:id", UangMakanControllerV2.getById);
